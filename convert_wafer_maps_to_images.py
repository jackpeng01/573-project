...
def main() -> None:
    ...
    print(
        f"Found {len(df_defective)} defective wafers and {len(df_nondefective)} non-defective wafers."
    )
    ...
    df_defective_sampled = df_defective.sample(
        n=min(num_defective, len(df_defective)), random_state=42
    )
    df_nondefective_sampled = df_nondefective.sample(
        n=min(num_nondefective, len(df_nondefective)), random_state=42
    )
    ...
    for _, row in tqdm(
        df_defective_sampled.iterrows(),
        total=len(df_defective_sampled),
        desc="Defective",
    ):
    ...
    for _, row in tqdm(
        df_nondefective_sampled.iterrows(),
        total=len(df_nondefective_sampled),
        desc="Non-defective",
    ):
        wafer_map = row["waferMap"]
        if wafer_map is None or np.all(np.array(wafer_map) == 0):
